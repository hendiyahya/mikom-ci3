...
class Hendi_login {
	...
	*/
	...
	public function login($username, $password) {
		...
		$query = $this->CI->db->get_where('hendi_users',array('username'=>$username,'password' => md5($password)));
		 
		if($query->num_rows() == 1) {
			//ambil data user berdasar username
			$row  = $this->CI->db->query('SELECT id_user FROM hendi_users where username = "'.$username.'"');
			$admin     = $row->row();
			$id   = $admin->id_user;
